...
def orcid_handler(signal, sender, instance=None, **kwargs):
    ...
    for attribute_map in settings.ORCID_PROVIDER_MAP:
        if 'orcid_autocomplete' in attribute_map and instance.attribute.uri == attribute_map.get('orcid_autocomplete'):
            ...
            Value.objects.update_or_create(
                ...
                snapshot=None,
                attribute=Attribute.objects.get(uri=attribute_map.get('type')),
            ...
            for key, path in [
                ...
                if key in attribute_map:
                    Value.objects.update_or_create(
                        project=instance.project,
                        snapshot=None,
                        attribute=Attribute.objects.get(uri=attribute_map.get(key)),
                        set_prefix=instance.set_prefix,
                        set_index=instance.set_index,
            ...
            organization_attribute_keys = ['role', 'organization', 'ror']
            for set_index, employment in enumerate(employments):
                for i, e in enumerate(employment):
                    key = organization_attribute_keys[i]
                    if e is not None:
                        Value.objects.update_or_create(
                            project=instance.project,
                            snapshot=None,
